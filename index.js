...
function processSum(numberList, callback) {
  let array= callback(numberList.reduce(function(acc, value){
    return acc + value;
  },0))
  return array
}
...
function getFullNames(runners) {
  ...
  runners.forEach(function(item){
    return nameList.push(`${item.last_name}, ${item.first_name}`)
  })
  return nameList;
}
...
function firstNamesAllCaps(runners) {
  const runners1st = runners.map(function (people){
    return people.first_name.toUpperCase();
  });
  return runners1st;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const runnerSize = runners.filter(function(item){
...
function tallyUpDonations(runners) {
  const totDon = runners.reduce(function(accumulator, item){
